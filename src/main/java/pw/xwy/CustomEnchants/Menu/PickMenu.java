...
public class PickMenu {
	...
	static {
		...
		desc.clear();
		Utils.setItem(ChatColor.DARK_BLUE + "Hydro", Material.EMERALD, desc, 0, PickMenu);
		
		
		desc.clear();
		desc.add(ChatColor.AQUA + "This enchant allows your pickaxe to");
		desc.add(ChatColor.AQUA + "mine in a 3x3 area.");
		Utils.setItem("ExplosivePick", Material.FIREBALL, desc, 1, PickMenu);
		
		desc.clear();
		desc.add(ChatColor.AQUA + "Dying with this enchant on an item");
		desc.add(ChatColor.AQUA + "has a chance of keeping it on respawn");
		Utils.setItem("Soulbound", Material.GOLDEN_APPLE, 1, desc, 2, PickMenu);
		
		desc.clear();
		Utils.setItem(ChatColor.DARK_PURPLE + "Mystical", Material.DIAMOND, desc, 9, PickMenu);
		
		desc.clear();
		desc.add(ChatColor.AQUA + "This enchant brings great fortune to many");
		Utils.setItem("Fortune V", Material.DIAMOND_PICKAXE, desc, 10, PickMenu);
		
		desc.clear();
		Utils.setItem(ChatColor.BLUE + "Rare", Material.GOLD_INGOT, desc, 18, PickMenu);
		...
		Utils.setItem("Smelting", Material.GOLD_INGOT, desc, 19, PickMenu);
		
		desc.clear();
		desc.add(ChatColor.AQUA + "Mined blocks go straight to your inventory.");
		Utils.setItem("Magnet", Material.HOPPER, desc, 20, PickMenu);
		
		desc.clear();
		Utils.setItem(ChatColor.DARK_GREEN + "Uncommon", Material.IRON_INGOT, desc, 27, PickMenu);
		
		desc.clear();
		Utils.setItem(ChatColor.GREEN + "Common", Material.COAL, desc, 36, PickMenu);
		...
		Utils.setItem("Hardened", Material.WOOD_PICKAXE, desc, 37, PickMenu);
		
		desc.clear();
		Utils.setItem(ChatColor.BLUE + "Back", Material.NETHER_STAR, desc, 44, PickMenu);
	}
